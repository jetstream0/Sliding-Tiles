...
function revert_grid_state(grid, colors) {
...
function add_top() {
    ...
    new TextButton(canvas, "?", color_table.text_primary, color_table.text_hover, "1em Verdana", [size[0] - TOP_LEFT[0] - help_stagger, TOP_LEFT[1] - 10], () => {
        let help_popup = new Popup(canvas);
        ...
        let first_paragraph_y = help_popup.top_left[1] + header_x_offset + header_bottom_margin;
        let first_paragraph = new Paragraph(canvas, "The green tiles are movable. Click them, and use the arrow or WASD keys.", color_table.text_primary, paragraph_font, help_popup.width - max_width_sub, [help_popup.top_left[0] + left_margin, first_paragraph_y]);
        help_popup.children.push(first_paragraph);
        //aspect ratio is roughly 5 width x 2 height
        let gif_width = help_popup.width - 142;
        let gif_height = gif_width * 0.4;
        if (size[0] < 550) {
            gif_width *= 0.75;
            gif_height *= 0.75;
        }
        let first_gif_y = first_paragraph_y + first_paragraph.calculated_height - 4;
        ...
        let second_paragraph_y = first_gif_y + gif_height + 18;
        let second_paragraph = new Paragraph(canvas, "The green tiles will keep moving, even offscreen, unless blocked by another green tile. To win, move the green tiles into the blue outlined tiles, with all other tiles empty.", color_table.text_primary, paragraph_font, help_popup.width - max_width_sub, [help_popup.top_left[0] + left_margin, second_paragraph_y]);
        help_popup.children.push(second_paragraph);
        //second gif doesn't fit when canvas is very small
        if (size[0] >= 435) {
            let second_gif_y = second_paragraph_y + second_paragraph.calculated_height - 4;
...
function player_move(direction) {
    ...
    //check if player won
    let solution = steps[steps.length - 1];
    if (solved(grid_to_grid_colors(canvas_grid), solution)) {
        //won
        let win_popup = new Popup(canvas);
        //close button for popup
...
document.addEventListener("touchend", (e) => {
    ...
    if (!start_coords)
        return;
    let delta_x = e.changedTouches[0].clientX - start_coords[0];
    let delta_y = e.changedTouches[0].clientY - start_coords[0];
    console.log("delta", delta_x, delta_y);
    //ignore if movement is very little, or swipe not clearly in a direction
    let diff = Math.abs(Math.abs(delta_x) - Math.abs(delta_y));
    console.log("diff", diff);
    if (diff < 20 || (Math.abs(delta_x) < 20 && Math.abs(delta_y) < 20))
        return;
    //get direction
    let direction;
